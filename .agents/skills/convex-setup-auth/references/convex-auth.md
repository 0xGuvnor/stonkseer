...
5. Make sure the project has a configured Convex deployment:
   - run `bunx convex dev` first if `CONVEX_DEPLOYMENT` is not set
...
12. Run `bunx convex dev --once` or the normal dev flow to push the updated
...
- Treat `bunx @convex-dev/auth` as a required initialization step for existing
  apps, not an optional extra

## Concrete Steps

1. Install `@convex-dev/auth` and `@auth/core@0.37.0`
2. Run `bunx convex dev` if the project does not already have a configured
   deployment
3. If `bunx convex dev` blocks on interactive setup, ask the user explicitly to
   finish configuring the Convex deployment
4. Run `bunx @convex-dev/auth`
...
9. Run `bunx convex dev --once` or the normal dev flow so the updated schema and
...
- frontend app entry such as `src/main.tsx` or the framework-equivalent provider
  file
- generated Convex Auth setup produced by `bunx @convex-dev/auth`
...
- `bunx @convex-dev/auth` is important because it initializes the auth setup,
  including the key material. Do not skip it when adding Convex Auth to an
  existing project.
- `bunx @convex-dev/auth` will fail if the project does not already have a
  configured `CONVEX_DEPLOYMENT`.
- `bunx convex dev` may require interactive setup for deployment creation or
  project selection. If that happens, ask the user explicitly for that human
  step instead of guessing.
- `bunx @convex-dev/auth` does not finish the whole integration by itself. You
...
If `bunx convex dev` or deployment setup requires human input:
...
- Run `bunx convex dev --once` or the normal dev flow after setup changes and
...
- [ ] Run `bunx convex dev` first if needed
...
- [ ] Run `bunx convex dev --once` or the normal dev flow after setup changes
