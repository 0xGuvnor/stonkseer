          ...
          <div className="rounded-3xl border bg-card p-6 shadow-sm">
            <div className="mb-5 flex items-start justify-between gap-4">
              ...
              results.events.length > 0 ? (
                <Button onClick={handleSave}>Save all to portfolio</Button>
              ) : null}
            </div>
                  ...
                  {results.events.map((event: CatalystEventView) => {
                    ...
                    return (
                      <TableRow key={event._id}>
                        <TableCell
                          className="max-w-[220px] whitespace-normal align-top font-medium"
      ...
      <aside className="rounded-3xl border bg-card p-6 shadow-sm">
        ...
        <Show when="signed-out">
          <p className="text-sm leading-6 text-muted-foreground">
            Try one ticker first. When you want to save catalysts, sign in with
            Google and create a portfolio.
          </p>
        </Show>
        <Show when="signed-in">
